...
export function PublicProofPage() {
  ...
  const handleNativeShare = async () => {
    ...
    try {
      if (navigator.canShare && navigator.canShare({ title: 'InLinkr Proof', text: shareText, url: currentUrl })) {
  ...
  const handleDownloadImage = async () => {
    ...
    try {
      ...
      canvas.toBlob((blob) => {
        if (!blob) return;
        const fileName = `inlinkr-proof-${token}.png`;
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex gap-2 justify-center mt-6">
            ...
            <button
              ...
              className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 sm:py-3 bg-primary hover:bg-primary text-white text-sm font-medium rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed max-w-[140px]"
            >
